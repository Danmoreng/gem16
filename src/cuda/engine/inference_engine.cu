...
struct PrefillOffsets {
  ...
  std::uint64_t vision_patches = 0, vision_patch_normalized = 0;
  std::uint64_t vision_hidden_a = 0, vision_hidden_b = 0;
  std::uint64_t vision_positions = 0;
...
Status LaunchFp8ProjectionBatch(const std::uint8_t* activation, const float* scales,
  ...
  return internal::LaunchFp8CutlassProjectionBatch(
      activation, scales, binding.weight, binding.scales, output_bf16, tokens,
...
Status LaunchFp8QkvProjectionBatch(
    ...
    const Fp8Binding& q_binding, std::uint16_t* q_output_bf16,
    const Fp8Binding& k_binding, std::uint16_t* k_output_bf16,
    const Fp8Binding* v_binding, std::uint16_t* v_output_bf16,
    std::uint64_t tokens,
  ...
  Status status = internal::LaunchFp8CutlassProjectionBatch(
      activation, scales, q_binding.weight, q_binding.scales,
      q_output_bf16, tokens, q_binding.rows, q_binding.contracting,
      cutlass_workspace,
      cutlass_workspace_bytes, stream);
  if (!status.ok()) return status;
  status = internal::LaunchFp8CutlassProjectionBatch(
      activation, scales, k_binding.weight, k_binding.scales,
      k_output_bf16, tokens, k_binding.rows, k_binding.contracting,
      cutlass_workspace,
      cutlass_workspace_bytes, stream);
  if (!status.ok() || v_binding == nullptr) return status;
  return internal::LaunchFp8CutlassProjectionBatch(
      activation, scales, v_binding->weight, v_binding->scales,
      v_output_bf16,
...
Status InferenceEngine::InitializeShared(const internal::LoadedTargetModel& model, const internal::AssistantModel* assistant, std::uint64_t max_context, KvCacheMode kv_cache_mode, const SamplingOptions& sampling, std::uint32_t mtp_draft_tokens) { return impl_->InitializeShared(model, assistant, max_context, kv_cache_mode, sampling, mtp_draft_tokens); }
...
Result<std::uint32_t> InferenceEngine::PrefillAt(std::span<const std::uint32_t> token_ids, std::uint64_t start_position, std::span<float> host_logits, std::span<const AudioEmbeddingSegment> audio_segments, std::span<const VisionEmbeddingSegment> vision_segments, GenerationCancellation cancellation) { return impl_->PrefillAt(token_ids, start_position, host_logits, audio_segments, vision_segments, cancellation); }
