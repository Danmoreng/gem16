...
parts with `/audio <path>` and `/image <path>` until the next text message;
`/media` lists them and `/clear-media` removes them. Their command-line or queue
order is preserved. The pinned miniaudio decoder accepts WAV,
