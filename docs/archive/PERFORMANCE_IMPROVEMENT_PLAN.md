> Historical performance plan. This file does not authorize new tuning. Consult [current decisions](../ACTIVE_DECISIONS.md).

# Gemma 4 12B final performance sprint

Status: S00-S08, the Windows regression, and the direct all-regions memory-reserve record are complete; the
optimization sequence is closed
...
In all cases, raw evidence is retained. The subsequent bounded Linux short-context ordinary-decode investigation
also closed without a production change after both admitted candidates failed. Its tracked record is
`benchmarks/baselines/llama_cpp/linux-short-context-065b68f.json`; Gemma 4 26B A4B M00 is now active.
