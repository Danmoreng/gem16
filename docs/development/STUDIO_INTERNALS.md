> Detailed implementation and test reference. Start with the [Studio user guide](../STUDIO.md).

# Native Studio implementation reference
...
- Explicit SHA-256 re-verification runs on a worker, reports byte progress, supports cancellation, and invalidates failed verification markers without deleting shared payloads.
- Failed and cancelled exchanges retain partial text for display and are excluded from subsequent canonical model history; retry sends the original user turn again.
