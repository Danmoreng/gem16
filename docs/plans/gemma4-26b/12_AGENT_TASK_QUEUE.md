...
M00–M17 and M22 are accepted. Bounded prefill, M21 and M20 are the active technical-close sequence; full M19 is owner-deferred.
...
| 1 | bounded prefill slice | active next | profile first; retain only a correctness-preserving win before final evidence |
| 2 | M20/M21 runner contracts and clean freeze | after prefill | make M20 consume native M21 evidence; freeze one hash |
| 3 | M21 real context | after clean freeze | repeat 32K, execute 64K and measure `base_max_context` |
| 4 | M20 controlled performance | after M21 | approved bounded 3-warm-up/10-retained run with telemetry |
