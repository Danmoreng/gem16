...
- `tools/gem16_compile/quantize_q4_0.py`
- `tests/unit/q4_0_test.cpp`
- `tests/cuda/embedding_head_test.cu`

## Implementation sequence

1. Implement the exact Q4_0 block contract: 32 weights, one FP16 scale, 16 packed bytes, canonical nibble order and deterministic rounding.
2. Implement an NVFP4 encoder for the same matrix using the M06 codec and a dedicated embedding/head role.
...
4. Compare project Q4_0 bytes against the official tensor where tensor mapping and source revision permit exact comparison; otherwise compare dequantized values and document why byte identity is not expected.
...
```text
python tools/compile_gemma4_26b.py --source-lock models/gemma4-26b-qat-bf16.lock.json --stage embedding-head --head-format q4_0 --output build/models/qat-head-q4
```
```text
python tools/compile_gemma4_26b.py --source-lock models/gemma4-26b-qat-bf16.lock.json --stage embedding-head --head-format nvfp4 --output build/models/qat-head-nvfp4
```
```text
python tools/benchmark_gemma4_26b_head.py --formats bf16,q4_0,nvfp4 --warmups 3 --repetitions 10 --output artifacts/m07/head-format-performance.json
```
...
Read repository AGENTS.md, docs/DECISIONS.md, docs/CORRECTNESS.md, docs/BENCHMARKING.md, the package master plan, and this milestone. Work only on M07. Inspect the actual current tree before editing and write a drift note if it differs from the anchored commit.
