...
class ResponseChannelTracker {
 public:
  ...
  [[nodiscard]] std::size_t open_match_length() const { return open_match_length_; }
  [[nodiscard]] std::uint64_t reasoning_token_count() const { return reasoning_token_count_; }
...
class Tokenizer {
 ...
 private:
  explicit Tokenizer(std::shared_ptr<const Impl> implementation) : implementation_(std::move(implementation)) {}
...
class GemmaChatProcessor {
 public:
  [[nodiscard]] static Result<GemmaChatProcessor> Load(const std::filesystem::path& model_directory);

  [[nodiscard]] Result<std::string> Render(std::span<const ChatMessage> messages, bool enable_thinking,
                                           bool add_generation_prompt = true,
                                           std::span<const ChatToolDefinition> tools = {}) const;
  [[nodiscard]] Result<std::vector<std::uint32_t>> Encode(std::span<const ChatMessage> messages, bool enable_thinking,
                                                          bool add_generation_prompt = true,
                                                          std::span<const ChatToolDefinition> tools = {}) const;
 ...
 private:
  GemmaChatProcessor(Tokenizer tokenizer, GenerationTokenControls controls, std::string thinking_open,
                     std::string thinking_close, std::vector<std::string> content_close_tokens,
